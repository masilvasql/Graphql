...
func (c *Course) FindByCourseId(courseId *string) (Course, error) {
	var id, name, description string
	err := c.db.QueryRow("SELECT id, name, description FROM courses WHERE id = $1", courseId).Scan(&id, &name, &description)
	if err != nil {
		return Course{}, err
	}
	return Course{
